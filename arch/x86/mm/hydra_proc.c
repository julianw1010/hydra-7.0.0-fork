...
static const struct hydra_int_knob hydra_route_prefetch_knob = {
	"route_prefetch", &sysctl_hydra_route_prefetch, 0, 1,
};
...
static int __init hydra_proc_init(void)
{
	...
	if (!proc_create_data("route_prefetch", 0644, hydra_dir, &hydra_knob_ops,
			      (void *)&hydra_route_prefetch_knob))
		goto fail;
