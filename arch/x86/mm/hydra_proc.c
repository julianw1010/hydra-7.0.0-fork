...
static const struct hydra_int_knob hydra_promote_knob = {
	"promote", &sysctl_hydra_promote, 0, 1,
};

static const struct hydra_int_knob hydra_promote_ms_knob = {
	"promote_ms", &sysctl_hydra_promote_ms, 50, 60000,
};
...
static int __init hydra_proc_init(void)
{
	...
	if (!proc_create_data("promote", 0644, hydra_dir, &hydra_knob_ops,
			      (void *)&hydra_promote_knob))
		goto fail;

	if (!proc_create_data("promote_ms", 0644, hydra_dir, &hydra_knob_ops,
			      (void *)&hydra_promote_ms_knob))
		goto fail;
