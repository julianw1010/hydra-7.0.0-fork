				...
				unsigned long addr, int master_node)
{
	...
	master_pmd = hydra_walk_to_pmd(mm, addr, master_node);
	...
	if (pmd_trans_huge(*master_pmd)) {
		...
		hydra_stats_eager_huge(mm);

		for (node = 0; node < NUMA_NODE_COUNT; node++) {
			unsigned long prefetched;
			int ret;

			if (node == master_node || !node_online(node))
				continue;

			current->hydra_eager_active = 1;
			ret = hydra_repl_pmd_range(mm, vma, addr, node,
		...
		for (node = 0; node < NUMA_NODE_COUNT; node++) {
			...
			current->hydra_eager_active = 1;
			ret = hydra_repl_pte_range(mm, vma, addr & PMD_MASK, 0,
						   PTRS_PER_PTE, node,
						   master_node, 0);
			current->hydra_eager_active = 0;
...
{
	pmd_t *npmd = hydra_walk_to_pmd(mm, addr, nid);

	if (HYDRA_WALK_BAD(npmd) || pmd_none(*npmd) || pmd_trans_huge(*npmd) ||
	    pmd_bad(*npmd))
		return;

	if (pmd_pfn(*npmd) != page_to_pfn(page))
		return;

	native_set_pmd(npmd, __pmd(0));
}

static void hydra_shed_clear_pmd_parent(struct mm_struct *mm, unsigned long addr,
					int nid, struct page *page)
{
	pgd_t *pgd = pgd_offset_node(mm, addr, nid);
	p4d_t *p4d;
	pud_t *pud;
	...
	if (pud_pfn(*pud) != page_to_pfn(page))
		return;

	native_set_pud(pud, __pud(0));
}

static void hydra_shed_fold_pte(struct mm_struct *mm, pmd_t *master_pmd,
				unsigned long addr, struct page *replica)
{
	pte_t *mbase = (pte_t *)((unsigned long)pte_offset_kernel(master_pmd, addr)
				 & PAGE_MASK);
	pte_t *rbase = (pte_t *)page_address(replica);
	spinlock_t *ptl = pte_lockptr(mm, master_pmd);
	int i;

	spin_lock(ptl);
	for (i = 0; i < PTRS_PER_PTE; i++) {
		pte_t rv = READ_ONCE(rbase[i]);
		pte_t old, new;

		if (!pte_present(rv))
			continue;
		if (!pte_young(rv) && !pte_dirty(rv))
			continue;

		old = READ_ONCE(mbase[i]);
		do {
			if (!pte_present(old))
				break;
			new = old;
			if (pte_young(rv))
				new = pte_mkyoung(new);
			if (pte_dirty(rv)) {
				new = pte_set_flags(new, _PAGE_DIRTY);
				new = pte_mksaveddirty(new);
			}
			if (pte_same(new, old))
				break;
		} while (!try_cmpxchg((long *)&mbase[i].pte,
				      (long *)&old, *(long *)&new));
	}
	spin_unlock(ptl);
}

static void hydra_shed_fold_pmd(struct mm_struct *mm, pmd_t *master_pmd,
				struct page *replica)
{
	pmd_t *mbase = (pmd_t *)((unsigned long)master_pmd & PAGE_MASK);
	pmd_t *rbase = (pmd_t *)page_address(replica);
	spinlock_t *ptl = pmd_lockptr(mm, master_pmd);
	int i;

	spin_lock(ptl);
	for (i = 0; i < PTRS_PER_PMD; i++) {
		pmd_t rv = READ_ONCE(rbase[i]);
		pmd_t old, new;

		if (!pmd_present(rv) || !pmd_leaf(rv))
			continue;
		if (!pmd_young(rv) && !pmd_dirty(rv))
			continue;

		old = READ_ONCE(mbase[i]);
		do {
			if (!pmd_present(old) || !pmd_leaf(old))
				break;
			new = old;
			if (pmd_young(rv))
				new = pmd_mkyoung(new);
			if (pmd_dirty(rv)) {
				new = pmd_set_flags(new, _PAGE_DIRTY);
				new = pmd_mksaveddirty(new);
			}
			if (pmd_same(new, old))
				break;
		} while (!try_cmpxchg((long *)&mbase[i],
				      (long *)&old, *(long *)&new));
	}
	spin_unlock(ptl);
}

static void hydra_shed_ring(struct mm_struct *mm, pmd_t *master_pmd,
			    unsigned long addr, int level,
			    struct list_head *sink, long *freed)
{
	struct page *master, *cur, *next;

	if (level == HYDRA_PT_PTE)
		master = virt_to_page(pte_offset_kernel(master_pmd, addr));
	else
		master = virt_to_page(master_pmd);

	if (!master || !READ_ONCE(master->next_replica))
		return;

	hydra_chain_lock(master);
	cur = master->next_replica;
	master->next_replica = NULL;
	...
	while (cur && cur != master) {
		...
		if (level == HYDRA_PT_PTE) {
			hydra_shed_fold_pte(mm, master_pmd, addr, cur);
			hydra_shed_clear_pte_parent(mm, addr, nid, cur);
			mm_dec_nr_ptes(mm);
		} else {
			hydra_shed_fold_pmd(mm, master_pmd, cur);
			hydra_shed_clear_pmd_parent(mm, addr, nid, cur);
			mm_dec_nr_pmds(mm);
		}

		list_add(&cur->lru, sink);
		(*freed)++;
...
static long hydra_shed_mm(struct mm_struct *mm)
{
...

	{
		...
		for_each_vma(vmi, vma) {
			int master_node = vma->master_pgd_node;
			unsigned long addr;

			for (addr = vma->vm_start & PMD_MASK;
			     addr < vma->vm_end; addr += PMD_SIZE) {
				pmd_t *mpmd = hydra_walk_to_pmd(mm, addr,
								master_node);

				if (HYDRA_WALK_BAD(mpmd) || pmd_none(*mpmd) ||
				    pmd_trans_huge(*mpmd) || pmd_bad(*mpmd))
					continue;

				hydra_shed_ring(mm, mpmd, addr, HYDRA_PT_PTE,
						&sink, &freed);
			}
		}
	}

	{
		...
		for_each_vma(vmi, vma) {
			...
			for (addr = vma->vm_start & PUD_MASK;
			     addr < vma->vm_end; addr += PUD_SIZE) {
				pmd_t *mpmd = hydra_walk_to_pmd(mm, addr,
								master_node);

				if (HYDRA_WALK_BAD(mpmd))
					continue;

				hydra_shed_ring(mm, mpmd, addr, HYDRA_PT_PMD,
						&sink, &freed);
			}
		}
	}
	...
	list_for_each_entry_safe(page, tmp, &sink, lru) {
		list_del(&page->lru);
		hydra_free_chain_node_rcu(page);
	}
...
int hydra_set_eager(pid_t pid, int mode)
{
	...
	WRITE_ONCE(mm->eager_repl_enabled, mode == HYDRA_EAGER_ON);
	if (mode == HYDRA_EAGER_OFF_SHED)
		freed = hydra_shed_mm(mm);
